    ...
    <#assign activ = operation.activation.substring(6).trim() >
		...
		<#if (operation.warning ?? ) && (operation.warning != "") >
				...
				if (res != JOptionPane.OK_OPTION){
					if (app.showCancelMessage()){
						JOptionPane.showMessageDialog(null, app.cancelMessage(), "", JOptionPane.INFORMATION_MESSAGE,SwingUtils.INFO_ICON);				
					}
					return;
				}
		</#if>	
		<#if operation.beanName ?? >
			...
			<#if operation.method??>
				...
				<#if method.ret?? && method.ret.toString().trim()!="" >
					ctx.put("${method.ret}",
...
<#--	public JComponent getPanel(Contexto ctx2) {
		...
		<#--
		<#list window.getBeans() as bean >
		if(${bean.name?uncap_first} instanceof Servicio){
					((Servicio)${bean.name?uncap_first}).setContext(ctx);
					 ctx.put("${bean.name}", ${bean.name?uncap_first});
				}
  		 </#list>
	...
	public JComponent getPanel(Contexto ctx2) {
		...
		if(panelPrincipal == null){
			...
			panel.setOpaque(false);
			this.panelPrincipal = panel;
			HelpUtil.asociaHelp(panelPrincipal,"${conversation.id}.${window.id}");
			panelPrincipal.putClientProperty("HELPID","${conversation.id}.${window.id}");
			}
